...
export async function GET() {
  ...
  // Require an explicit successful check for any configured dependency.
  // - If a database URL is present, database must be 'ok'.
  // - If the check code exists but the URL is missing, we set 'not_configured' → unhealthy.
  // - If the database check block was removed entirely, the key is absent → ok for this dimension.
  const dbHealthy = !('database' in health) || health.database === 'ok';
