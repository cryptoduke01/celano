...
export async function POST(req: Request) {
  ...
  // Idempotency — atomically claim the event using the unique constraint.
  // This prevents duplicate processing under concurrent webhook deliveries.
  const claimed = await tryClaimEvent(webhookId);
  if (!claimed) {
  ...
  try {
    ...
    // Event is already durably claimed; no need to mark again.
  } catch (err) {
    // Log but still return 200 — otherwise Dodo retries endlessly.
    // The claim above ensures retries will see the event as processed.
...
```typescript
// Atomically claim the event. Returns true if we are the first to process it.
async function tryClaimEvent(webhookId: string): Promise<boolean> {
  try {
    await db.processedWebhook.create({ data: { webhookId } });
    return true; // We claimed it — proceed to handle.
  } catch (e: any) {
...
For Supabase without an ORM (atomic insert + unique violation detection):
```typescript
async function tryClaimEvent(webhookId: string): Promise<boolean> {
  const { error } = await supabase
    .from('processed_webhooks')
    .insert({ webhook_id: webhookId });
  if (!error) return true; // Inserted — we claimed it.
  // Postgres unique violation (23505) means it was already processed.
